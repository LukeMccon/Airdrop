...
- The classifier tests exhaustively deny every other `ClickType`/`InventoryAction` pair exposed by the test runtime. MockBukkit 3.133.2 resolves Paper API 1.21.1 on the test classpath, so Paper 1.21.8 bundle actions remain part of the manual runtime matrix.
