# Airdrop 5 replaces implementation access with one supported service

Airdrop 5 introduces the first versioned extension boundary. Integrations that
used controllers, managers, `Crate`, raw configuration objects, or
`com.airdropmc.events` should move to `com.airdropmc.api`.
...
`PackageDropEvent` and `PackageLandEvent` remain deprecated adapters during the
Airdrop 5 major line. New integrations should listen under
`com.airdropmc.api.event`:
